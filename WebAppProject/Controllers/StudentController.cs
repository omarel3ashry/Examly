...
namespace WebAppProject.Controllers
{
    ...
    public class StudentController : Controller
    {
        ...
        [HttpPost]
        public IActionResult SubmitExam(List<Choice> choices,int examId)
        {
            int grade=0;
            int id = int.Parse(User.Claims.FirstOrDefault(c => c.Type == "id")!.Value);
            studentRepository.AddStudentAnswers(examId, id, choices);
            List<ExamChoices> examChoices = studentRepository.GetStudentAnswers(id, examId);
            foreach (var item in examChoices)
            {
                

                if (item.CorrectChoices.Count == item.StudentChoices.FindAll(e=>e.IsCorrect).Count&& item.CorrectChoices.Count == item.StudentChoices.Count )
                {
